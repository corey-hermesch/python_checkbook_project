...
def are_commas_ok(amount_str):
    
    # goal is to accept '1,000' or '1,000.00' and reject '1,00' for example
    # first, just in case it hasn't been done, remove the '$' and whitespace
    amount_str = amount_str.strip('$ ')
    # remove the decimal and everything to the right of decimal
    if '.' in amount_str:
        # before removing everything to the right, check for commas to the right
        dec_index = amount_str.index('.')
        if ',' in amount_str[dec_index+1:]:
            return False
        amount_str = amount_str[:amount_str.index('.')]
    ...
    if comma_count == 0:
        ...
        i = 0
        while i < comma_count:
            # compare index of comma to next comma index or end of string
            comma_index = amount_str.index(',')
            # if there are not exactly 3 digits after the comma return False
            if not amount_str[comma_index+1:comma_index+4].isdigit():
                return False
            # if there are exactly 3 digits after the comma and that's the end of the string -> True
            elif len(amount_str) == comma_index + 4:
                return True
            # if the 4th character after the comma is not another comma, -> False
            elif amount_str[comma_index+4] != ',':
                return False
            # if there are exactly 3 digits after the first comma, remove it and iterate
            amount_str = amount_str[:comma_index] + amount_str[comma_index+1:]
            i += 1
    return True
...
def get_valid_money(amount_str):
    ...
    while True:
        ...
        try:
            ...
            amount_str = amount_str.strip('$ ')                  
            # check to see if commas are in the correct position(s)
            if are_commas_ok(amount_str):
